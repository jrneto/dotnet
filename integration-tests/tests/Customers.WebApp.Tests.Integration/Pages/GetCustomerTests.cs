...
namespace Customers.WebApp.Tests.Integration.Pages
{
    ...
    public class GetCustomerTests
    {
        ...
        private readonly Faker<Customer> _customerGenerator = new Faker<Customer>()
            ...
            .RuleFor(x => x.GitHubUsername, faker => SharedTestContext.ValidGitHubUsername)
        ...
        public async Task Get_ReturnsNoCustomer_WhenNoCustomerExists()
        {
            ...
            var custumerUrl = $"{SharedTestContext.AppUrl}/customer/{Guid.NewGuid()}";

            // Act
            await page.GotoAsync(custumerUrl!);

            // Assert

            (await page.Locator("p").First.InnerTextAsync()).Should().Be("No customer found with this id");
        }
